...
type MapProps = {
  ...
  loading?: boolean;
  /** Show switch to toggle between OpenStreetMap and Google Maps (default: true) */
...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  const [googleCenter, setGoogleCenter] = useState<[number, number] | null>(null);
  const [googleZoom, setGoogleZoom] = useState<number>(13);

  const handleSwitchToGoogle = useCallback(() => {
    if (mapInstance) {
      const center = mapInstance.getCenter();
      setGoogleCenter([center.lng, center.lat]);
      setGoogleZoom(Math.round(mapInstance.getZoom()));
    } else if (viewport?.center) {
      setGoogleCenter(viewport.center as [number, number]);
      setGoogleZoom(viewport.zoom ?? 13);
    }
    setProvider("google");
  }, [mapInstance, viewport]);

  const getGoogleMapsUrl = useCallback(() => {
    const lon = googleCenter ? googleCenter[0] : (viewport?.center?.[0] ?? 77.2090);
    const lat = googleCenter ? googleCenter[1] : (viewport?.center?.[1] ?? 28.6139);
    const zoom = googleZoom;
    return `https://maps.google.com/maps?q=${lat},${lon}&t=&z=${zoom}&ie=UTF8&iwloc=&output=embed`;
  }, [googleCenter, googleZoom, viewport]);
  ...
  return (
    <MapContext.Provider value={contextValue}>
      ...
      >
        ...
        {showProviderToggle && (
          <div className="absolute z-30 flex items-center gap-1 rounded-full border border-border/80 bg-background/90 p-1 shadow-md backdrop-blur-md bottom-3 left-3 sm:bottom-auto sm:left-auto sm:top-3 sm:right-12">
            <button
              type="button"
              onClick={() => setProvider("osm")}
              className={cn(
                "flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-bold transition-all cursor-pointer",
                provider === "osm"
        ...
        {/* Google Maps Embed iframe Overlay */}
        {provider === "google" && (
          <div className="absolute inset-0 z-20 bg-background w-full h-full overflow-hidden">
            <iframe
